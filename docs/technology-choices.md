...
| **Base de données**    | MongoDB                                   | *POC simple et flexible (JSON), sans schéma strict. Collections :* `telemetry`*,* `posture_events`*. Indexation possible (ex. par `deviceId` et horodatage) pour requêtes temps réel + historique.* |
